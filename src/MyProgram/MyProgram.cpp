...
public:
    ...
    void decreaseNumberOfCyborgs(int n) {
        ...
        if (n <= getNumberOfCyborgs()) {
            ...
            throw (std::string("Invalid decrease of cyborgs from") + to_string(getNumberOfCyborgs()) + " by " +
                   to_string(n) + " at factory " + to_string(id));
        }
    }
...
virtual bool MyProgram::setup(std::string) final override {
...
int main() {
    ...
    for (int i = 0; i < linkCount; i++) {
        ...
        int factory2;
        int distance;
        cin >> factory2 >> distance >> factory1;
        cin.ignore();
